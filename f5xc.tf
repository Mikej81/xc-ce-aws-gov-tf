...
resource "volterra_securemesh_site_v2" "this" {
  ...
  # Site mesh group connectivity over public IP on SLO.
  # Post-registration manual steps are required — see README.
...
resource "terraform_data" "set_public_ip" {
  ...
  triggers_replace = [aws_eip.slo[0].public_ip]
  ...
  provisioner "local-exec" {
    ...
    command = <<-SCRIPT
      ...
      PUBLIC_IP="${aws_eip.slo[0].public_ip}"
      MAX_WAIT=3600
      POLL_INTERVAL=30
